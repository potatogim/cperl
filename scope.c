...
Perl_markstack_grow(pTHX)
{
    ...
    DEBUG_s(PerlIO_printf(Perl_debug_log, "MARK grow %p %" IVdf " by %" IVdf "\n",
...
Perl_save_scalar(pTHX_ GV *gv)
{
    ...
    DEBUG_lv(Perl_deb(aTHX_ "save SV %s\n",  SvPEEK(*sptr)));
...
Perl_save_shared_pvref(pTHX_ char **str)
{
    ...
    DEBUG_lv(Perl_deb(aTHX_ "save SHARED_PVREF \"%s\"\n", *str));
...
Perl_save_set_svflags(pTHX_ SV* sv, U32 mask, U32 val)
{
    ...
    SS_ADD_END(4);
    DEBUG_lv(Perl_deb(aTHX_ "save SET_SVFLAGS\n"));
}
...
Perl_save_gp(pTHX_ GV *gv, I32 empty)
{
    ...
    DEBUG_lv(Perl_deb(aTHX_ "save GP\n"));
...
Perl_save_ary(pTHX_ GV *gv)
{
    ...
    DEBUG_lv(Perl_deb(aTHX_ "save AV [%ld]\n", (long)AvFILLp(oav)));
...
Perl_save_hash(pTHX_ GV *gv)
{
    ...
    save_pushptrptr(SvREFCNT_inc_simple_NN(gv), (ohv = GvHVn(gv)), SAVEt_HV);
    GvHV(gv) = NULL;
    hv = GvHVn(gv);
    DEBUG_lv(Perl_deb(aTHX_ "save HV %s %s\n", SvPEEK(ohv), SvPEEK(hv)));
...
Perl_save_bool(pTHX_ bool *boolp)
{
    ...
    SS_ADD_END(2);
    DEBUG_lv(Perl_deb(aTHX_ "save BOOL %d\n", (int)*boolp));
}
...
Perl_save_int(pTHX_ int *intp)
{
    ...
    if (UNLIKELY((int)(type >> SAVE_TIGHT_SHIFT) != i)) {
        ...
        type = SAVEt_INT;
        size++;
        DEBUG_lv(Perl_deb(aTHX_ "save INT %d\n", (int)i));
    } else {
        DEBUG_lv(Perl_deb(aTHX_ "save INT_SMALL %d\n", (int)i));
    }
    SS_ADD_PTR(intp);
    SS_ADD_UV(type);
    SS_ADD_END(size);
...
Perl_save_I8(pTHX_ I8 *bytep)
{
    ...
    SS_ADD_END(2);
    DEBUG_lv(Perl_deb(aTHX_ "save I8 %d\n", (int)*bytep));
}
...
Perl_save_I16(pTHX_ I16 *intp)
{
    ...
    SS_ADD_END(2);
    DEBUG_lv(Perl_deb(aTHX_ "save I16 %d\n", (int)*intp));
}
...
Perl_save_I32(pTHX_ I32 *intp)
{
    ...
    if (UNLIKELY((I32)(type >> SAVE_TIGHT_SHIFT) != i)) {
        ...
        type = SAVEt_I32;
        size++;
        DEBUG_lv(Perl_deb(aTHX_ "save I32 %d\n", (int)i));
    } else {
        DEBUG_lv(Perl_deb(aTHX_ "save I32_SMALL %d\n", (int)i));
    }
    SS_ADD_PTR(intp);
    SS_ADD_UV(type);
    SS_ADD_END(size);
...
Perl_save_strlen(pTHX_ STRLEN *ptr)
{
    ...
    SS_ADD_END(3);
    DEBUG_lv(Perl_deb(aTHX_ "save STRLEN %ld\n", (long)*ptr));
}
...
Perl_save_padsv_and_mortalize(pTHX_ PADOFFSET off)
{
    ...
    SS_ADD_END(4);
    DEBUG_lv(Perl_deb(aTHX_ "save PADSV_AND_MORTALIZE %s [%d]\n",
                      SvPEEK(PL_curpad[off]), (int)off));
}
...
Perl_save_delete(pTHX_ HV *hv, char *key, I32 klen)
{
    ...
    DEBUG_lv(Perl_deb(aTHX_ "save DELETE\n"));
}
...
Perl_save_hdelete(pTHX_ HV *hv, SV *keysv)
{
    ...
    DEBUG_lv(Perl_deb(aTHX_ "save HDELETE\n"));
}
...
Perl_save_adelete(pTHX_ AV *av, SSize_t key)
{
    ...
    SS_ADD_END(3);
    DEBUG_lv(Perl_deb(aTHX_ "save ADELETE\n"));
}
...
Perl_save_destructor(pTHX_ DESTRUCTORFUNC_NOCONTEXT_t f, void* p)
{
    ...
    SS_ADD_END(3);
    DEBUG_lv(Perl_deb(aTHX_ "save DESTRUCTOR\n"));
}
...
Perl_save_destructor_x(pTHX_ DESTRUCTORFUNC_t f, void* p)
{
    ...
    SS_ADD_END(3);
    DEBUG_lv(Perl_deb(aTHX_ "save DESTRUCTOR_X\n"));
}
...
Perl_save_hints(pTHX)
{
    ...
    DEBUG_lv(Perl_deb(aTHX_ "save HINTS\n"));
}
			    ...
			    const U32 flags)
{
    ...
    SS_ADD_END(4);
    DEBUG_lv(Perl_deb(aTHX_ "save AELEM\n"));
...
Perl_save_helem_flags(pTHX_ HV *hv, SV *key, SV **sptr, const U32 flags)
{
    ...
    SvGETMAGIC(*sptr);
    {
        ...
        SS_ADD_END(4);
        DEBUG_lv(Perl_deb(aTHX_ "save HELEM\n"));
    }
...
Perl_save_svref(pTHX_ SV **sptr)
{
    ...
    DEBUG_lv(Perl_deb(aTHX_ "save SVREF\n"));
    return save_scalar_at(sptr, SAVEf_SETMAGIC); /* XXX - FIXME - see #60360 */
}
...
Perl_save_alloc(pTHX_ I32 size, I32 pad)
{
    ...
    DEBUG_lv(Perl_deb(aTHX_ "save ALLOC\n"));
...
Perl_leave_scope(pTHX_ I32 base)
{
	...
	TAINT_NOT;
        ...
        if (DEBUG_l_TEST) {
            if (DEBUG_v_TEST)
                PerlIO_printf(Perl_debug_log, " (type=%ld, ix=%ld)\n",
                              (long)type, (long)ix);
            else
	...
	case SAVEt_ITEM:			/* normal string */
            a0 = ap[0]; a1 = ap[1];
            DEBUG_lv(Perl_deb(aTHX_ "restore ITEM %p %p\n", a1.any_sv, a0.any_sv));
	...
	case SAVEt_GENERIC_PVREF:		/* generic pv */
	    ...
	    if (*a1.any_pvp != a0.any_pv) {
                DEBUG_lv(Perl_deb(aTHX_ "restore GENERIC_PVREF %p '%p'\n",
	...
	case SAVEt_SHARED_PVREF:		/* shared pv */
	    ...
	    if (*a0.any_pvp != a1.any_pv) {
                DEBUG_lv(Perl_deb(aTHX_ "restore SHARED_PVREF '%s' '%s'\n",
                                  a1.any_pv, *a0.any_pvp));
#ifdef NETWARE
	...
	case SAVEt_TMPSFLOOR:			/* restore PL_tmps_floor */
            a0 = ap[0];
            DEBUG_lv(Perl_deb(aTHX_ "restore TMPSFLOOR %ld => %ld\n",
                              (long)PL_tmps_floor, (long)a0.any_iv));
	...
	case SAVEt_DELETE:
            a0 = ap[0]; a1 = ap[1]; a2 = ap[2];
            DEBUG_lv(Perl_deb(aTHX_ "restore DELETE %p{%s:%d}\n",
                              a2.any_hv, a0.any_pv, (int)a1.any_i32));
	...
	case SAVEt_ADELETE:
            a0 = ap[0]; a1 = ap[1];
            DEBUG_lv(Perl_deb(aTHX_ "restore ADELETE %p[%ld]\n",
                              a1.any_av, (long)a0.any_iv));
	...
	case SAVEt_DESTRUCTOR_X:
            a0 = ap[0]; a1 = ap[1];
            DEBUG_lv(Perl_deb(aTHX_ "restore DESTRUCTOR_X %p(%p)\n",
                              *a0.any_dxptr, a1.any_ptr));
	...
	case SAVEt_STACK_POS:		/* Position on Perl stack */
            a0 = ap[0];
            DEBUG_lv(Perl_deb(aTHX_ "restore STACK_POS %d\n",
                              (int)a0.any_i32));
	...
	case SAVEt_AELEM:		/* array element */
	    ...
	    svp = av_fetch(a0.any_av, a1.any_iv, 1);
            DEBUG_lv(Perl_deb(aTHX_ "restore AELEM %p[%ld] (%p) => %p\n",
                              a0.any_av, (long)a1.any_iv, svp?*svp:NULL, a2.any_sv));
	...
	case SAVEt_HELEM:		/* hash element */
	    ...
	    he = hv_fetch_ent(a0.any_hv, a1.any_sv, 1, 0);
            DEBUG_lv(Perl_deb(aTHX_ "restore HELEM %p{%p} (%p) => %p\n",
                              a0.any_hv, a1.any_sv, he, a2.any_sv));
	    SvREFCNT_dec(a1.any_sv);
	    if (LIKELY(he)) {
	...
	case SAVEt_OP:
            ...
            DEBUG_lv(Perl_deb(aTHX_ "restore OP %s => %p\n",
                              PL_op ? OP_NAME(PL_op) : "(null)", a0.any_ptr));
	    PL_op = (OP*)a0.any_ptr;
	    break;

	case SAVEt_HINTS:
            a0 = ap[0]; a1 = ap[1];
            DEBUG_lv(Perl_deb(aTHX_ "restore HINTS %p %d\n",
                              a1.any_ptr, (int)a0.any_i32));
	    if ((PL_hints & HINT_LOCALIZE_HH)) {
	...
	case SAVEt_COMPPAD:
            a0 = ap[0];
            DEBUG_lv(Perl_deb(aTHX_ "restore COMPPAD %p\n", a0.any_ptr));
	...
	case SAVEt_PADSV_AND_MORTALIZE:
		...
		svp = AvARRAY((PAD*)a1.any_ptr) + (PADOFFSET)a2.any_uv;
                DEBUG_lv(Perl_deb(aTHX_ "restore PADSV_AND_MORTALIZE %p[%ld] %p\n",
                                  a1.any_ptr, (long)a2.any_uv, a0.any_sv));
	...
	case SAVEt_SAVESWITCHSTACK:
		...
		dSP;
                ...
                a0 = ap[0]; a1 = ap[1];
                DEBUG_lv(Perl_deb(aTHX_ "restore SAVESWITCHSTACK %p %p\n",
                                  a1.any_av, a0.any_av));
	...
	case SAVEt_I8:				/* I8 reference */
            a0 = ap[0];
            DEBUG_lv(Perl_deb(aTHX_ "restore I8 %p %d\n",
                              a0.any_ptr, (int)(I8)(uv>>8)));
	...
	case SAVEt_COMPILE_WARNINGS:
            a0 = ap[0];
            DEBUG_lv(Perl_deb(aTHX_ "restore COMPILE_WARNINGS %p\n",
                              a0.any_ptr));
