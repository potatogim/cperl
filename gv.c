			...
			const U32 flags)
{
    ...
    tmpbuf[0] = '_';
    tmpbuf[1] = '<';
    memcpy(tmpbuf + 2, name, namelen+1);
    gv = *(GV**)hv_fetch(PL_defstash, tmpbuf, tmplen, TRUE);
    if (!isGV(gv)) {
	gv_init(gv, PL_defstash, tmpbuf, tmplen, FALSE);
#ifdef PERL_DONT_CREATE_GVSV
	GvSV(gv) = (SV*)newSVpvn(name, namelen);
#else
	sv_setpvn((SV*)GvSV(gv), name, namelen);
#endif
    }
...
The only significant values for C<flags> are C<GV_SUPER>, C<SVf_UTF8>,
and C<GV_CLASS>.

C<GV_SUPER> indicates that we want to look up the method in the superclasses
of the C<stash>.

C<GV_CLASS> allows for non-existing methods to be searched in classes
or readonly packages. Normally a failing lookup will throw a
restricted hash error.
...
S_gv_fetchmeth_internal(pTHX_ HV* stash, PV* meth, const char* name,
                        STRLEN len, I32 level, U32 flags)
{
    ...
    DEBUG_o( Perl_deb(aTHX_ "Looking for %smethod %s in %s %s\n",
		      flags & GV_SUPER ? "SUPER " : "",
		      name ? name : SvPV_nolen(meth),
                      HvPKGTYPE_NN(stash), hvname) );
    ...
    while (items--) {
        ...
        DEBUG_ov(Perl_deb(aTHX_ "Looking for %smethod %s in %s %" HEKf "\n",
		      flags & GV_SUPER ? "SUPER " : "",
		      name ? name : SvPV_nolen(meth),
        ...
        if (!gvp) {
            if (len > 1 && HvNAMELEN_get(cstash) == 4) {
                ...
                if (memEQc(hvname, "CORE")
                    && (candidate = S_maybe_add_coresub(aTHX_ cstash,NULL,name,len)))
                    goto have_candidate;
            }
            continue;
        }
...
need to check for it being C<"AUTOLOAD">, since at the later time the
call may load a different subroutine due to C<$AUTOLOAD> changing its
...
Perl_gv_fetchmethod_pvn_flags(pTHX_ HV *stash, const char *name, const STRLEN len, U32 flags)
{
    ...
    const U32 is_utf8  = flags & SVf_UTF8;
    const U32 ifexists = flags & GV_CLASS;
    bool is_import;
...
Perl_gv_autoload_pvn(pTHX_ HV *stash, const char *name, STRLEN len, U32 flags)
{
	...
	if (SvTYPE(stash) < SVt_PVHV) {
	    ...
	    packname = (PV*)sv_2mortal(newSVhek(HvNAME_HEK(stash)));
	...
	if (SvPOK(cv)) { /* Ouch! */
	    PV * const tmpsv = newSVpvn_flags(name, len, is_utf8);
	    STRLEN ulen;
	    ...
	    sv_catpvn_flags(tmpsv, proto, CvPROTOLEN(cv),
                            SV_CATBYTES*!SvUTF8(cv));
	    SvTEMP_on(tmpsv); /* Allow theft */
	    sv_setsv_nomg((SV*)cv, (SV*)tmpsv);
	    SvTEMP_off(tmpsv);
    ...
    vargv = *(GV**)hv_fetch_ifexists(varstash, S_autoload, S_autolen, TRUE);
    ENTER;
    ...
    LEAVE;
    varsv = GvPVn(vargv);
    SvTAINTED_off((SV*)varsv); /* previous $AUTOLOAD taint is obsolete */
    ...
    sv_catpvn_flags(varsv, name, len,
                        ...
                        const STRLEN len, const U32 flags)
{
    ...
    if (!target || !SvRMAGICAL(target)
     || !mg_find(target, varname == '[' ? PERL_MAGIC_tiedscalar
                                        : PERL_MAGIC_tied))
    {
<<<<<<< d3a3aaf51a1d4efe7cc8e7da9ff88d260ea5a71b
        HV *stash;
        GV **gvp;
        dSP;
...
#define HV_FETCH_TIE_FUNC (GV **)hv_fetchs_ifexists(stash, "_tie_it", 0)
...
#define HV_FETCH_TIE_FUNC (GV **)hv_fetchs(stash, "_tie_it", 0)
      ...
      if (!(stash = gv_stashpvn(name, len, 0))
       || !(gvp = HV_FETCH_TIE_FUNC) || !*gvp || !GvCV(*gvp))
      {
	PV * const module = newSVpvn(name, len);
	const char type = varname == '[' ? '$' : '%';
	if ( flags & 1 )
	    save_scalar(gv);
	Perl_load_module(aTHX_ PERL_LOADMOD_NOIMPORT, module, NULL);
	...
	if (!stash)
      ...
      call_sv((SV *)*gvp, G_VOID|G_DISCARD);
      LEAVE;
>>>>>>> CowREFCNT: change SV* to PV* in most APIs
    }
}
...
S_gv_stashsvpvn_cached(pTHX_ PV *namesv, const char *name, U32 namelen, I32 flags)
{
    ...
    he = (HE *)hv_common(PL_stashcache, namesv, name, namelen,
		...
		if (strEQc(name, "\003HILD_ERROR_NATIVE"))
		    goto magicalize;
                ...
                if (memEQs(name, len, "\003APTURE")) {
                    ...
                    sv_magic(MUTABLE_SV(av), INT2PTR(SV*,uv), PERL_MAGIC_regdata, NULL, 0);
                    SvREADONLY_on(av);
...
Perl_Gv_AMupdate(pTHX_ HV *stash, bool destructing)
{
    ...
    AMT amt;
    U32 newgen;
    bool is_closed = SvREADONLY(MUTABLE_SV(stash));
    ...
    DEBUG_o( Perl_deb(aTHX_ "Recalcing overload magic in %s %s\n",
                      HvPKGTYPE_NN(stash),
                      HvNAME_get(stash)) );
...

    {
        ...
        /* Try to find via inheritance. First "$CLASS::x()" */
        ...
        for (i = 1; i < NofAMmeth; i++) {
            ...
            DEBUG_o( Perl_deb(aTHX_ "Checking overloading of \"%s\" in %s \"%.256s\"\n",
                              cp, HvPKGTYPE_NN(stash), HvNAME_get(stash)) );
            ...
            if (gv && (cv = GvCV(gv)) && CvHASGV(cv)) {
                     ...
                     && strEQc(HEK_KEY(stashek), "overload") )
                {
                    ...
                    DEBUG_o( Perl_deb(aTHX_ "Resolving method \"%" SVf256 \
                        "\" for overloaded \"%s\" in %s \"%.256s\"\n",
                        (void*)GvSV(gv), cp, HvPKGTYPE_NN(stash), HvNAME(stash)) );
                    if (!gvsv || !SvPOK(gvsv)
                ...
                DEBUG_o( Perl_deb(aTHX_
                    "Overloading \"%s\" in %s \"%.256s\" via \"%.256s::%.256s\"\n",
                     cp, HvPKGTYPE_NN(stash), HvNAME_get(stash), HvNAME_get(GvSTASH(CvGV(cv))),
                     GvNAME(CvGV(cv))) );
                filled = 1;
    ...
    /* Here we have no table, cache the empty amt. */
 no_table:
    AMT_AMAGIC_off(&amt);
    if (is_closed)
        SvFLAGS(stash) &= ~SVf_READONLY;
    sv_magic(MUTABLE_SV(stash), 0, PERL_MAGIC_overload_table,
