...
=item B<< c3 as mro default >>

cperl uses since 5.26 the standard c3 mro as default, the old dfs is
available in F<ext/mro> as module.

c3 is a bit stricter when loading incompatible, not resolvable C<@ISA> chains.
Some modules either need to change their wrong C<@ISA>, or C<use mro "dfs";>
in subclassed packages. The most common error is B<Exporter> at the beginning
of the C<@ISA> list of a subclassed package, where it needs to be at the end.
...
The fix is in Math::BigInt::Subclass:
...
The @ISA of classes and packages using base or fields is now closed
(i.e. readonly), you cannot change it run-time. Hereby enabling some
compile-time optimizations, and avoiding wrong monkey patching
in-between, where it should happen at the end of the inheritance chain.
