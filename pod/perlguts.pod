...
C<HeHASH(HE* he)> is not supported anymore, as the hash is now
computed and not stored in the HE. Ditto for C<SvSHARED_HASH(sv)>.
Both return 0.
C<HEK_HASH(hek)> is now undefined.
...
calling C<sv_force_normal(sv)> or SvPV_force_nolen(sv).
...
think it should be (i.e., you need it for your extension), send a report via
L<cperlbug> explaining why you think it should be.
...
in the C<pTHX_> macro indicates that the macro expansion needs a comma
after the context argument because other arguments follow it.  If
PERL_IMPLICIT_CONTEXT is not defined, C<pTHX_> will be ignored, and the
...
C<Perl_warner>), or use a context-free version.

The context-free version of Perl_warner is called
C<Perl_warner_nocontext>, and does not take the extra argument.  Instead
...
=head2 Various lengths and index types

Internally perl uses various different lengths for different data
types, which will vary for 32bit and 64bit systems, but are only
dependent on the architecture, not customziable by changing the C<IV>
and C<UV> types.

A PV string has C<STRLEN> (i.e. max 32/64bit), a hash key only C<I32>.
Names are usually stored as hash key, hence max C<I32_MAX> (31bit).
Arrays and hashes allow C<SSize_t> indices, which is 31/63bit.

A refcounted_he (used to store hints for lexical pragmas) has the length
C<U32> with threads and C<STRLEN> without.
...
Mixing UTF-8 and non-UTF-8 strings is tricky.  Use C<bytes_to_utf8> to
...
Stephen McCamant, Reini Urban and Gurusamy Sarathy.
