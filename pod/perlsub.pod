...
To declare external subroutines:
X<subroutine, extern> X<extern> X<ffi>

    extern sub NAME (SIG) :ATTRS;            # signature must contain types
    extern sub NAME (SIG);                   # default return type
    sub NAME (SIG) :native;                  # no library name
    sub NAME (SIG) :native("mysqlclient");   # constant library name
    sub NAME (SIG) :native($libname);        # variable library name
    sub NAME (SIG) :native :symbol("cname"); # other c symbol name
...
Aside from L</Signatures> Perl does not have named formal
parameters.  In practice all you do is assign to a C<my()> list of
these.  Variables that aren't declared to be private are global
variables.  For gory details on creating private variables, see
L</"Private Variables via my()"> and L</"Temporary Values via
local()">.  To create protected environments for a set of functions in
a separate package (and probably a separate file), see
L<perlmod/"Packages">.  X<formal parameter> X<parameter, formal>
...
B<WARNING>: Subroutine signatures are experimental in Perl5, not in
cperl.  The feature may be modified or removed in future versions of
Perl.
...
known as a I<signature>.
In cperl signatures are enabled by default, in Perl 5 the feature must
be enabled first by C<use feature 'signatures'>, and might produce a
...
equivalent to
    ...
    sub foo {
	die "Wrong number of arguments for subroutine" unless @_ != 2;
	my $left = $_[0];
	my $right = $_[1];
	return $left + $right;
    }

An argument can be ignored by omitting the main part of the
name from a parameter declaration, leaving just a bare C<$> sigil.
For example,
...
In cperl optional paramaters can also be declared as in Perl 6 with a C<'?'>
suffix, which will default to undef.
...
In Perl5 only an optional parameter can be nameless just like a mandatory
...
many arguments may be passed.

In Perl5 a slurpy array parameter may be nameless just like a
...
In Perl5 a slurpy hash parameter may be nameless just like other kinds of
...
In Perl5 the empty C<()> prototype is used to denote constant foldable
functions, and signatures can only be declared in the C<use feature
'signatures'> scope.  In cperl the empty C<()> list denotes always
an empty signature, and all functions which can be constant-folded are
constant folded. No C<()> is needed.

A parameter by reference does not copy the variable into the body, it
allows destructive modification of the variable. This is the same as
using the old C<$_[0]> syntax:
...
When using a signature, all processed arguments are not available in
the special array variable C<@_> anymore, as if they are shifted from
C<@_>. There is a difference between the two ways of accessing the
...
which is not referenced only changes that variable, and has no effect
on the caller's variables, but using a reference variable or without
signatures writing to an element of C<@_> modifies whatever the caller
used to supply that argument.
...
In Perl5 to avoid ambiguity, when signatures are enabled the special syntax
...
In cperl you can declare deviating C<:prototype($)> strings, but the
default is the prototype which matches the signature types. Prototypes
were initially designed in 2001 to allow future signatures, but the
problem is that p5p forgot about that, and forces users to write a lot
more useless boilerplate declarations.
...
the signature. With cperl this is optional.

As with lexical variable declarations you may declare the types of
parameters, and you may declare the return type of the subroutine with
a final type attribute:

    sub foo (int $left, str $right) :int {
	return $left + int($right);
    }

The types may also be declared as attribute

    sub foo ($left :int, $right :str) :int {
	return $left + int($right);
    }

Those types must be existing classes or packages. Those additional
types declarations help the compiler to produce better code and to
throw early compile-time errors if possible.

With methods the first parameter, the C<$self> invocant does not need to be declared.
For another variable than the default C<$self>, the C<:> invocant suffix is used:
...
C<$self> is not added with C<sub> and the C<:method> attribute.

    sub baz ($self, $bar?) :method {
	return $self->{baz};
    }


With extern or :native subroutine declarations the types in the
signature are mandatory, and the block is empty. Some FFI specific
types are imported via the L<ffi> module. The functions are searched
in all loaded shared libraries, a new shared library must be specified
via the :native argument, are loaded beforehand via
C<DynaLoader::dl_load_file()>.
An alternate name for the C function may be declared via the :symbol
argument.

    extern sub rand() :int;

    use ffi;
    extern sub srand(uint $i) :void;

    sub mysql_fetch_row ($result :ptr)
        :native("mysqlclient", 20) :long;

The :native attribute accepts a string for the library name, searched
via C<dl_load_file()>, plus an optional numeric argument for
the soname version to find it easier on windows-like platforms without
symlink support, and an optional int for the soname version.
See L<attributes/native>.
...
scoped--magical built-ins like C<$/> must currently be localized
...
With subroutine signatures prototypes are automatically extracted from the
...
Functions declared with the attribute C<:const> are compile-time evaluated and
inlined.
...
The following functions would all be inlined:
    ...
    sub PI :const	{ 4 * atan2 1, 1 }	# compile-time evaluated
...
With cperl the C<:const> attribute is not experimental, and can be
applied to named functions and methods also, when the signature or
prototype is empty. See also L</Pure Functions> for inlinable
functions with arguments. Note that methods can only be inlined if the
dispatch can be resolved at compile-time, i.e. when the class is
I<final> (e.g. C<use oo :final; class :const> or C<const @ISA and
const %class::>) and the object is typed.
...
Note that with B<cperl> this C<return> statement or the C<()> prototype does
not prevent the function from being inlined if it's otherwise inlinable.
You will be able to use the C<:-inline> attribute there (I<NYI>).

    sub not_inlined () :-inline { 23 }
    sub not_inlined :-inline { return 23 }
    sub not_inlined :-inline { 23 }

=head2 Pure Functions
X<:pure>

With cperl you can declare functions as C<:pure> to denote that the
body will be side-effect free (i.e. I<purely functional>), and thus
the return value can be cached depending on the arguments to avoid
unncessary run-time calculations. In fact many internal ops are
declared as C<:pure>.

    my $a:const = 1;
    sub add_to_a($x) :pure { $x+$a }
    print add_to_a(3) for 1..3;
