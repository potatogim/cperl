...
Not yet implemented are the :encoded() attribute for string arguments
...
%CLASS::" errors. Stash lookup now got a GV_CLASS flag and hv_fetch*_ifexists
macro, which does the EXISTS and FETCH hash actions in the stash on one.
...
Under DEBUGGING
"Modification of a read-only value attempted at <location>"
was changed to 
"Modification of a read-only value attempted <value> at <c file:line> at <location>"
...
new C<croak_no_modify_sv(sv)> has better diagnostics and is used
throughout core.
