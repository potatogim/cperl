...
C<default>, C<DESTROY>, C<else>, C<elseif>, C<elsif>, C<END>,
C<__END__>, C<eq>, L<C<extern>|/"extern sub NAME ();">, C<for>,
C<foreach>, C<ge>, C<given>, C<gt>, C<if>, C<INIT>, C<le>, C<lt>,
C<ne>, C<not>, C<or>, C<UNITCHECK>, C<unless>, C<until>, C<when>,
C<while>, C<x>, C<xor>.

See L<perlsyn/"Statement Modifiers"> and L<perlsyn/"Compound Statements">
...
=item extern sub NAME ();
X<ffi> X<extern>

=for Pod::Functions declare an external ffi subroutine

This is a declaration of an external FFI subroutine, with a mandatory
signature. The signature must declare the incoming types to uniquely match
the possible C types, the return type declaration defaults to C<:long>.

C<extern> may be replaced by the L<C<:native>|attributes/native> attribute.
