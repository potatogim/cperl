...
or L<C<values>|/values HASH> on the hash or array.

Destructive operations on the hash keys while iterating over it is now
disabled by default with B<cperl 5.24.0>. Enable the previous behavior
with C<use hashiter;> If you add or delete a hash's elements while
iterating over it with C<use hashiter>, the effect on the iterator is
unspecified; for example, entries may be skipped or duplicated--so
don't do that.  Exception: With L<C<use hashiter>|hashiter> it is safe
to delete the item most recently returned by L<C<each>|/each HASH>, so
...
a recent vintage:
    ...
    use cperl; use hashiter; # to allow destructive changes in an each() loop.
