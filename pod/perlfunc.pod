...
With UNEXEC enabled this dumps the current state of the interpreter to
a native executable, without UNEXEC it causes an immediate core dump.
See also the B<-u> command-line switch in L<perlrun>, which does the
same thing.

The optional EXPR denotes either a computed LABEL or the name of
resulting executable. If the LABEL exists or no argument is
provided the name of the executable is created with the same name is
with C<-u>, i.e. F<scriptname.exe> or F<a.exe>.

Without UNEXEC this is so that you can use the B<undump> program (not
supplied) to turn your core dump into an executable binary after
having initialized all your variables at the beginning of the program.
When the new binary is executed it will begin by executing a C<goto
LABEL> (with all the restrictions that L<C<goto>|/goto LABEL>
suffers).  Think of it as a goto with an intervening core dump and
reincarnation.  If C<LABEL> is omitted, restarts the program from the
top.  The C<dump EXPR> form, available starting in Perl 5.18.0, allows
a name to be computed at run time, being otherwise identical to C<dump
LABEL>.

B<WARNING>: Any files opened at the time of the dump will I<not>
be open any more when the program is reincarnated, with possible
resulting confusion by Perl. Any IO actions at BEGIN, like C<chdir> is also
very problematic.

This function is largely obsolete in perl5 without UNEXEC, mostly
because it's very hard to convert a core file into an executable.
That's why you should now invoke it as C<CORE::dump()> if you don't
want to be warned against a possible typo.
However in B<cperl>, C<dump> and C<-u> are very convenient ways to create
simple, unoptimized binaries, much faster than with B<perlcc>.
...
Returns the prototype or signature of a function as a string (or
L<C<undef>|/undef EXPR> if the function has no prototype).  FUNCTION
is a reference to, or the name of, the function whose prototype you
want to retrieve.  If FUNCTION is omitted, L<C<$_>|perlvar/$_> is
used.
...
Signatures are not returned as declared, rather as parsed, without whitespace.
Signatures are embedded in C<"("> and C<")">, so you can easily seperate them from
signature-less prototypes.
Default expressions are denoted as "<expr>", constants as such.
L<:prototype|perlsub/Prototypes> attributes override signatures as result of the
C<prototype> function.

    sub empty { }
    prototype(\&empty) == undef;
    ...
    sub func1 ($a = 222) { }
    prototype(\&func1) eq '($a=222)';

    sub func2 ($a = $x, $b = $x++) { }
    prototype(\&func2) eq '($a=$x, $b=<expr>)';

    sub func3 ($a?) { }
    prototype(\&func1) eq '($a=)';

    sub func4 ($a) : prototype($) { }
    prototype(\&func0) eq '$';
...
As of cperl v5.26 or Perl 5.28, this special-cased whitespace
splitting works as expected in the scope of L<< S<C<"use feature
'unicode_strings">>|feature/The 'unicode_strings' feature >>. In
previous versions, and outside the scope of that feature, it exhibits
L<perlunicode/The "Unicode Bug">: characters that are whitespace
...
=for Pod::Functions possibly optimize the internal representation

C<study> on a hash optimizes the internal structure. On other values
stringifies it and calls get magic. Note that the perl5 documentation
incorrectly states that study is a no-op.
On arrays, subroutines or regexp it does nothing yet, but may help in
supporting sparse arrays or jit compile regular expressions later.
With hashes it only clears the placeholders, later it might optimize
readonly hashes to perfect hashes.
