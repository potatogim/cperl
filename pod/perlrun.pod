...
B<perl>	S<[ B<-sTtUWX> ]>
	...
	S<[ B<-cw> ] [ B<-c:s> ] [ B<-d>[B<t>][:I<debugger>] ] [ B<-D>[I<number/list>] ]>
...
causes Perl to check the syntax of the program in a safe compartment,
without any undoable side-effects and then exit without executing it.
The safe variant to C<-c>.
...
B<-D14> is equivalent to B<-Dtls>):
 ...
 67108864  L  trace Locale-related info; what gets output is very
              subject to change
 134217728 I  PerlIO debugging (previously via env PERLIO_DEBUG)
...
Without UNEXEC you can then in theory take this core
dump and turn it into an executable file by using the I<undump>
program (not supplied).

If the optional filename after C<-u=> is omitted the resulting binary file
is named after the given script file with the F<.exe> extension appended.
Without script, just C<-e'...'> use F<a.exe> as filename.
...
execute a portion of your program before dumping, use the L<perlfunc/dump>
operator instead.  Note: availability of I<undump> or I<UNEXEC> is
platform specific and may not be available for a specific port of
Perl.

In B<cperl>, C<dump> and C<-u> are very convenient ways to create
simple, unoptimized binaries, much faster than with L<perlcc>.
...
If set to the name of a file or device, C<perl -D> debug messages will
not be printed to C<STDERR> but will be appended to this file. This way
you can seperate debug messages from warnings.

In older perls C<PERLIO_DEBUG> was available even with non debugging
perl.  This is now only available with C<DEBUGGING> and PerlIO-only
debugging is available via the new switch C<-DI>.

Typical uses are in Unix:

   % env PERLIO_DEBUG=/dev/tty perl -DoI script ...
...
With B<cperl> C<PERL_PERTURB_KEYS> is hardcoded by default to C<"0">
and B<"TOP">, which disables the default perl5 traversal
randomization. On each lookup the key is moved to the front of the
collison chain, for subsequent faster lookups of the same key.

With C<PERL_PERTURB_KEYS_DISABLED> being defined C<PERL_PERTURB_KEYS>
is hardcoded to C<"0"> and B<"DISABLED">, which also disables the
default perl5 traversal randomization.
