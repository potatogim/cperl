	...
	if (is_first)
	    Perl_warner(aTHX_ packWARN(WARN_SYNTAX),
		    "\t(Missing semicolon on previous line?)\n");
                 ...
                 && isIDFIRST_lazy_if_safe(PL_oldoldbufptr, PL_bufend, UTF))
        {
	    const char *t;
  ...
  switchdefault:
	...
	}
    {
        ...
        if (len > UNRECOGNIZED_PRECEDE_COUNT) {
            d = UTF ? (char*)utf8_hop_back((U8*) s, -UNRECOGNIZED_PRECEDE_COUNT,
                                       (U8*)PL_linestart)
                    : s - UNRECOGNIZED_PRECEDE_COUNT;
        } else {
            d = PL_linestart;
        }
        Perl_croak(aTHX_  "Unrecognized character %s; marked by <-- HERE after %" UTF8f
                   "<-- HERE near column %d", c,
                   UTF8fARG(UTF, (s - d), d),
                   (int) len + 1);
    }
    case 4:
...
#define doPOWcop(len,val)                               \
	...
	case XOPERATOR:
		...
		d = scan_word(d, PL_tokenbuf + 1, sizeof PL_tokenbuf - 1,
			      FALSE, &len, &normalize);
                if (UNLIKELY(normalize)) {
                    char *s1 = pv_uni_normalize(PL_tokenbuf+1, len, &len);
                    Copy(s1, &PL_tokenbuf[1], len+1, char);
                }
		while (d < PL_bufend && SPACE_OR_TAB(*d))
		    d++;
		if (*d == '}') {
	...
	d = s;
	{
		...
		if (!islop || PL_last_lop_op == OP_GREPSTART)
		    ...
		    char tmpbuf[sizeof PL_tokenbuf];
                    int normtmp; /* here ignored */
		    int t2;
		    scan_word(s, tmpbuf, sizeof tmpbuf, TRUE, &len, &normtmp);
	  ...
	  just_a_word: {
		...
		if ((*s == ':' && s[1] == ':')
			...
			Perl_croak(aTHX_ "Bad name after %" UTF8f "%s",
				...
				*s == '\'' ? "'" : "::");
                    if (UNLIKELY(normalize)) {
                        d = pv_uni_normalize(PL_tokenbuf + len, morelen, &morelen);
                        Copy(d, &PL_tokenbuf[len], morelen+1, char);
                    }
		    len += morelen;
		    pkgname = 1;
		}
                                                               ...
                                                               == OA_FILEREF))
		{
		    ...
		    if ((isIDFIRST_lazy_if_safe(s, PL_bufend, UTF) || *s == '$')
	...
	case KEY_foreach:
		...
		if (l >= 3 && memEQc(d, "my")
		    ...
		    d += 3;
                ...
                if (d != s && isIDFIRST_lazy_if_safe(d, PL_bufend, UTF)) {
                    int normalize;
...
#define VALIDATE_PROTO_WARN ckWARN(WARN_ILLEGALPROTO)
                    ...
                    s = skipspace(s);
                    if (have_proto) {
                        /* empty prototypes () are still valid sigs for
                           EXTERNSUB and METHOD */
                        if (!SvCUR(PL_lex_stuff)) {
                            /* method NAME () {} */
                            if ((PL_in_class && (cvflags & CVf_METHOD)) || is_extern) {
                                try_signature = TRUE; /* avoid 2nd sig stuffing below */
                                have_proto = FALSE;
                                DEBUG_T(printbuf("### Empty signature %s\n", d));
                                s = PL_bufptr = d;
                                if (!is_extern) {
                                    SV* sig = newSVpvn_flags(SvPVX(PL_curstname),
                                                             SvCUR(PL_curstname),
                            ...
                            if (UNLIKELY(is_extern))
                                goto extern_needs_sig;
                        }
                    } else {
		...
		force_ident_maybe_lex('&');
                if (is_extern)
                    ITOKEN(CVf_EXTERN|cvflags,EXTERNSUB); /* no body */
        ...
        if (is_utf8 && isIDFIRST_utf8_safe(*s, PL_bufend)) {
                   ...
                   isIDCONT_utf8_safe((const U8*) t, (const U8*) PL_bufend))
            {
                ...
                if (len > 1) {
                    ...
                    if (!*normalize && UNLIKELY(_is_decomposed_string((U8*)t, len)))
                        *normalize = 1;
                }
                t += len;
            }
